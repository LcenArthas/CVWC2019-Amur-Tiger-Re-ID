...
class BaseDataset(object):
    ...
    def get_imagedata_info(self, data, is_train=False):
        if is_train:
            pids, cams = [], []
            for _, pid, camid,_,_,_,_,_,_,_ in data:
                pids += [pid]
                cams += [camid]
            pids = set(pids)
            cams = set(cams)
...
class BaseImageDataset(BaseDataset):
    ...
    def print_dataset_statistics(self, train, query, gallery):
        num_train_pids, num_train_imgs, num_train_cams = self.get_imagedata_info(train, is_train=True)
